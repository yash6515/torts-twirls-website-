...
import { getAdminOrders, updateOrderStatus, updateShippingDetails } from '../../utils/api';
      ...
      <div className="relative bg-white rounded-3xl shadow-lifted w-full max-w-lg max-h-[90vh] overflow-y-auto animate-slide-up">
        ...
        <form onSubmit={handleSave} className="p-6 space-y-5">
          ...
          <div>
            ...
            />
            {form.trackingNumber && (
              <p className="text-[0.7rem] text-gold font-sans mt-1">
                ✦ Order will auto-advance to "Shipped" status when tracking is saved.
              </p>
            )}
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              onChange={handle}
              rows={3}
              placeholder="e.g. Fragile package, customer called for early delivery..."
...
const AdminOrders = () => {
  ...
  const [shippingOrder, setShippingOrder] = useState(null); // order being edited in modal
    ...
    <div className="min-h-screen bg-cream pt-[72px]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 animate-fade-in">
          <div>
            <p className="text-[0.7rem] font-sans uppercase tracking-[0.2em] text-gold mb-1">Management</p>
            <h1 className="font-display text-[2.2rem] text-deep-brown">Manage Orders</h1>
          </div>
        ...
        {/* Status filter tabs */}
        <div className="flex flex-wrap gap-2 mb-6 bg-white p-2 rounded-2xl shadow-card">
          <button
            onClick={() => setStatusFilter('')}
            className={`text-xs px-4 py-2 rounded-xl font-sans transition-all ${!statusFilter ? 'bg-deep-brown text-cream shadow-sm font-medium' : 'text-warm-gray hover:bg-cream hover:text-deep-brown'}`}
          >
            All Orders
          </button>
          {ALL_STATUSES.map(s => (
            <button
              ...
              className={`text-xs px-3 py-2 rounded-xl font-sans transition-all capitalize ${statusFilter === s ? 'bg-deep-brown text-cream shadow-sm font-medium' : 'text-warm-gray hover:bg-cream hover:text-deep-brown'}`}
            >
              {s.replace('_', ' ')}
            </button>
          ))}
                    ...
                    {orders.map((order, i) => (
                      ...
                      >
                        ...
                        <td className="px-5 py-4">
                          ...
                          {order.user?.phone && <p className="text-[0.68rem] text-taupe font-sans">{order.user.phone}</p>}
                        </td>
                            ...
                            <div className="flex -space-x-2">
                              {order.orderItems?.slice(0, 2).map((item, j) => (
                                <img
                                  ...
                                  onError={e => e.target.src = 'https://images.unsplash.com/photo-1631049307264-da0ec9d70304?w=60'}
                                />
                              ))}
                            </div>
                        ...
                        <td className="px-5 py-4">
                          {order.shippingDetails?.trackingNumber ? (
                            <div>
                              ...
                              <p className="text-[0.65rem] font-mono text-taupe mt-0.5">
                                {order.shippingDetails.trackingNumber}
                              </p>
                              {order.shippingDetails.trackingUrl && (
                                <a
                                  ...
                                  className="text-[0.65rem] text-gold hover:text-deep-brown font-sans link-underline"
                                >
                                  Track →
                                </a>
                              )}
                              {order.shippingDetails.estimatedDelivery && (
                                <p className="text-[0.65rem] text-taupe font-sans mt-0.5">
                                  Est: {new Date(order.shippingDetails.estimatedDelivery).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}
                                </p>
                              )}
                            </div>
                          ) : (
                            <span className="text-[0.68rem] text-taupe font-sans">Not dispatched</span>
                          )}
                        </td>
                        ...
                        <td className="px-5 py-4">
                          <div className="flex flex-col gap-1.5">
                            ...
                            <select
                              ...
                              className="text-[0.72rem] border border-sand rounded-lg px-2 py-1.5 font-sans text-deep-brown outline-none focus:border-taupe bg-white w-36 capitalize"
                            >
                              {ALL_STATUSES.map(s => (
