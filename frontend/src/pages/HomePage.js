    ...
    >
      ...
      {cat.tag && (
        <div className="absolute top-3 left-3 z-10">
          <span className="text-[0.6rem] font-sans uppercase tracking-widest bg-white/90 backdrop-blur-sm text-deep-brown px-2 py-0.5 rounded-full">
            {cat.tag}
          </span>
        </div>
      )}
      <div className="absolute bottom-0 left-0 right-0 p-3 sm:p-4 z-10">
        ...
        <p className="text-white/55 text-[0.65rem] font-sans mt-0.5">{cat.desc}</p>
        <div className="flex items-center gap-1 text-white/0 group-hover:text-sage/90 transition-all duration-300 mt-1.5 -translate-y-1 group-hover:translate-y-0">
          <span className="text-[0.65rem] font-sans">Shop Now</span>
...
const testimonials = [
  ...
  { name: 'Arjun Mehta', city: 'Delhi', rating: 5, text: 'Ordered king-size fitted sheets and they fit perfectly. The fabric quality is outstanding. Won\'t shop anywhere else.', avatar: 'A' },
  { name: 'Kavya Reddy', city: 'Bangalore', rating: 5, text: 'The linen duvet cover is so breathable even in summer. It has completely transformed my bedroom. Absolutely love it!', avatar: 'K' },
];
...
const HomePage = () => {
  ...
  return (
    <div className="pt-[72px]">
          ...
          <div className="grid lg:grid-cols-[1fr_460px] xl:grid-cols-[1fr_520px] gap-10 lg:gap-16 items-center">
            ...
            <div className="text-center lg:text-left">
              <div className={`transition-all duration-700 delay-100 ${heroLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
                <span className="inline-flex items-center gap-2 text-xs font-sans tracking-[0.18em] uppercase text-warm-gray border border-rose-primary/25 px-4 py-2 rounded-full bg-white/50 mb-6 sm:mb-8">
                  <span className="w-1.5 h-1.5 rounded-full bg-rose-primary animate-pulse-soft inline-block" />
              ...
              <div className={`transition-all duration-700 delay-200 ${heroLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
                <h1 className="font-display leading-[1.06] text-deep-brown mb-6" style={{ fontSize: 'clamp(2.5rem, 7vw, 5.5rem)' }}>
                  Sleep Like
                  <br />
                  <em className="italic gold-text">Royalty.</em>
                </h1>
              </div>

              <div className={`transition-all duration-700 delay-300 ${heroLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
                <p className="font-sans text-warm-gray text-[0.95rem] sm:text-[1.0625rem] leading-[1.75] mb-8 max-w-[440px] mx-auto lg:mx-0">
              ...
              <div className={`flex flex-wrap gap-2 justify-center lg:justify-start mb-8 transition-all duration-700 delay-[450ms] ${heroLoaded ? 'opacity-100' : 'opacity-0'}`}>
                {['✓ 100% Pure Cotton', '✓ OEKO-TEX Certified', '✓ 4.9 ★ Rating', '✓ Free Returns'].map(b => (
                  <span key={b} className="inline-block text-[0.7rem] sm:text-xs font-sans text-warm-gray border border-sand bg-white/70 px-3 py-1.5 rounded-full">
                    {b}
                  </span>
                ))}
              </div>
            ...
            <div className={`relative hidden lg:block transition-all duration-1000 delay-500 ${heroLoaded ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10'}`}>
              <div className="relative ml-auto w-full max-w-[460px] h-[580px] rounded-[2.5rem] overflow-hidden shadow-lifted">
                <img
                ...
                <div className="absolute bottom-6 left-5 right-5 bg-white/10 backdrop-blur-md border border-white/20 rounded-xl px-4 py-3">
                  <p className="text-white/70 text-[0.6rem] font-sans tracking-widest uppercase">Featured</p>
                  <p className="text-white font-display text-lg leading-tight">400 TC Egyptian Cotton</p>
                </div>
              </div>

              {/* Floating review */}
              <div className="absolute -bottom-4 -left-6 bg-white rounded-2xl shadow-lifted p-4 w-[190px] animate-float">
                ...
                <p className="text-[0.7rem] font-sans text-warm-gray italic leading-relaxed">"Like sleeping on a cloud!"</p>
              </div>

              {/* Thread count pill */}
              <div className="absolute top-8 -right-4 bg-rose-primary text-cream rounded-2xl px-4 py-3 shadow-glow animate-float" style={{ animationDelay: '1.5s' }}>
                <p className="text-[0.5rem] font-sans uppercase tracking-wider text-cream/60">Max Thread Count</p>
                <p className="font-display text-xl text-cream leading-none">1000 TC</p>
              </div>
            </div>
          </div>
        </div>

        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 opacity-35 hidden sm:flex pointer-events-none">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6">
            {[
              ...
              <div key={f.title} className="flex items-center gap-3 sm:gap-3.5 group">
                <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-full border border-rose-primary/20 bg-rose-mist/40 flex items-center justify-center flex-shrink-0 text-rose-primary group-hover:bg-rose-primary group-hover:text-white group-hover:border-rose-primary transition-all duration-300">
                  {f.icon}
                </div>
                <div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 sm:gap-3">
            {categories.map((cat, i) => (
              <div key={cat.slug} className={`reveal-on-scroll delay-${i + 1}`}>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10 sm:mb-14 reveal-on-scroll">
            ...
            <h2 className="font-display text-[2rem] sm:text-[2.8rem] text-deep-brown leading-tight mb-4">
              Crafted for <em className="italic gold-text">Exceptional Comfort</em>
            </h2>
            <p className="font-sans text-warm-gray text-[0.9rem] leading-relaxed max-w-lg mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 sm:gap-6 reveal-on-scroll">
            {[
              { icon: <IconSparkle />, title: 'Premium Long-Staple Cotton', desc: 'Longer fibres = smoother, stronger, softer sheets.' },
              { icon: <IconShield />, title: 'OEKO-TEX® Certified', desc: 'Tested for 100+ harmful substances. Safe for all skin.' },
              { icon: <IconSun />, title: 'Anti-Fade Technology', desc: 'Reactive dyes lock in colour through 200+ washes.' },
              { icon: <IconReturn />, title: '30-Day Easy Returns', desc: 'Not in love? Return it, no questions asked.' },
            ].map((f, i) => (
              <div key={i} className={`text-center p-6 rounded-2xl border border-sand/50 bg-cream/50 hover:shadow-card hover:-translate-y-1 transition-all duration-300 group delay-${i + 1}`}>
                <div className="w-12 h-12 rounded-xl bg-rose-mist border border-rose-primary/10 flex items-center justify-center mx-auto mb-4 text-rose-primary group-hover:bg-rose-primary group-hover:text-white transition-all duration-300">
                  {f.icon}
                </div>
                <p className="font-sans font-semibold text-deep-brown text-sm mb-1.5">{f.title}</p>
                <p className="text-warm-gray text-xs leading-relaxed">{f.desc}</p>
              </div>
            ))}
          </div>
      ...
      {/* ── STATS STRIP ── */}
      <section className="bg-rose-primary py-12 sm:py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 sm:gap-8 text-center">
            {[
              ...
              <div key={i}>
                <p className="font-display text-4xl sm:text-5xl text-cream font-light leading-none">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="relative rounded-3xl overflow-hidden reveal-on-scroll" style={{ minHeight: '420px' }}>
            <img
              ...
              onError={e => { e.target.src = 'https://images.unsplash.com/photo-1505693416388-ac5ce068fe85?w=1600'; }}
            />
            <div className="absolute inset-0 bg-gradient-to-r from-charcoal/90 via-chocolate/65 to-transparent" />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
            {testimonials.map((t, i) => (
              <div
                key={i}
                className={`bg-white rounded-2xl p-6 shadow-card reveal-on-scroll delay-${i + 1} hover:shadow-card-hover transition-all duration-400 hover:-translate-y-2 relative overflow-hidden group`}
              >
                <div className="absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r from-rose-primary via-rose-light to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-400" />
                <p className="font-display text-[5rem] sm:text-[6rem] leading-none text-rose-primary/8 -mb-8 select-none">"</p>
                <Stars n={t.rating} />
          ...
          <div className="mt-10 text-center reveal-on-scroll">
            <div className="inline-flex items-center gap-4 bg-white rounded-2xl px-8 py-4 shadow-soft">
              <div>
      ...
      <section className="py-16 sm:py-20 bg-deep-brown relative overflow-hidden">
        <div className="absolute inset-0 pointer-events-none" style={{
          backgroundImage: 'radial-gradient(circle at 20% 50%, rgba(107,174,140,0.08) 0%, transparent 50%), radial-gradient(circle at 80% 50%, rgba(64,145,108,0.06) 0%, transparent 50%)',
        }} />
        <div className="max-w-4xl mx-auto px-4 text-center relative z-10">
          ...
          <div className="flex flex-wrap justify-center gap-2.5 mb-10">
            {['OEKO-TEX® Certified', '100% Long-Staple Cotton', 'Anti-Fade Dyes', 'Sanforized Finish'].map(c => (
              <span key={c} className="text-xs font-sans text-cream/60 border border-cream/20 px-3.5 py-1.5 rounded-full bg-white/5">
                {c}
              </span>
            ))}
          </div>
