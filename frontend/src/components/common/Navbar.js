...
const Navbar = () => {
  ...
  return (
    <>
      {/* Scroll progress bar */}
      <div
        className="fixed top-0 left-0 z-[60] h-0.5 bg-gradient-to-r from-gold via-gold-light to-gold transition-all duration-150"
        style={{ width: `${scrollProgress}%` }}
      />

      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        scrolled
          ? 'bg-white/92 backdrop-blur-xl shadow-[0_1px_0_rgba(74,55,40,0.08),0_8px_24px_rgba(74,55,40,0.06)]'
          : 'bg-cream/80 backdrop-blur-md'
      }`}>

        {/* Announcement bar */}
        <div className={`bg-deep-brown text-cream text-center py-2 text-[0.72rem] font-sans tracking-[0.18em] uppercase transition-all duration-300 overflow-hidden ${
          scrolled ? 'max-h-0 py-0 opacity-0' : 'max-h-8 opacity-100'
        }`}>
          Free shipping on orders above ₹999 · Use code <span className="text-gold font-semibold">FIRST10</span> for 10% off
        </div>
          ...
          <div className="flex items-center justify-between h-[60px]">
            ...
            <div className="flex items-center gap-1 sm:gap-2">
              ...
              <button
                ...
                className="p-2.5 text-warm-gray hover:text-deep-brown hover:bg-sand/50 rounded-xl transition-all duration-200"
                aria-label="Search"
              >
              ...
              {/* Wishlist */}
              <Link to="/wishlist" className="relative p-2.5 text-warm-gray hover:text-deep-brown hover:bg-sand/50 rounded-xl transition-all duration-200">
                <svg className="w-[18px] h-[18px]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.75} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                </svg>
                {wishlist.length > 0 && (
                  <span className="absolute top-1 right-1 bg-rose-400 text-white text-[0.6rem] font-sans font-bold w-4 h-4 rounded-full flex items-center justify-center badge-pop">
              ...
              {/* Cart */}
              <Link to="/cart" className="relative p-2.5 text-warm-gray hover:text-deep-brown hover:bg-sand/50 rounded-xl transition-all duration-200">
                <svg className="w-[18px] h-[18px]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.75} d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
                </svg>
                {cartCount > 0 && (
                  <span className="absolute top-1 right-1 bg-deep-brown text-cream text-[0.6rem] font-sans font-bold w-4 h-4 rounded-full flex items-center justify-center badge-pop">
              ...
              {isAuthenticated ? (
                <div className="relative ml-1" ref={userMenuRef}>
                  <button
                    ...
                    className="flex items-center gap-2 pl-2 pr-3 py-1.5 rounded-xl hover:bg-sand/50 transition-all duration-200 group"
                  >
                    <div className="w-7 h-7 bg-gradient-to-br from-deep-brown to-warm-gray rounded-full flex items-center justify-center text-cream text-[0.72rem] font-semibold flex-shrink-0 shadow-sm">
                  ...
                  <div className={`absolute right-0 mt-2 w-56 bg-white rounded-2xl shadow-lifted border border-sand/40 overflow-hidden transition-all duration-300 origin-top-right ${
                    ...
                    {[
                      ...
                      <Link
                        key={item.path}
                        to={item.path}
                        className="block px-4 py-2.5 text-sm font-sans text-warm-gray hover:text-deep-brown hover:bg-cream transition-colors duration-150"
                      >
                        {item.label}
                      </Link>
                    ))}
                    {user?.role === 'admin' && (
                      <Link to="/admin" className="block px-4 py-2.5 text-sm font-sans text-gold hover:bg-gold-light/20 transition-colors duration-150 border-t border-sand/50">
                        ⚙️  Admin Panel
                      </Link>
                    )}
                    <button
                ...
                <Link
                  to="/login"
                  className="hidden sm:flex items-center gap-1.5 text-[0.8rem] font-sans text-deep-brown border border-deep-brown/25 px-4 py-2 rounded-xl hover:bg-deep-brown hover:text-cream hover:border-deep-brown transition-all duration-300 ml-1"
                >
                  Login
                </Link>
              )}
              ...
              <button
                ...
                className="md:hidden p-2.5 text-warm-gray hover:text-deep-brown hover:bg-sand/50 rounded-xl transition-all duration-200 ml-1"
      ...
      {/* Mobile menu overlay */}
      <div
        className={`fixed inset-0 z-40 md:hidden transition-all duration-400 ${menuOpen ? 'visible' : 'invisible'}`}
      ...
      {/* Mobile menu panel */}
      <div className={`fixed top-0 right-0 bottom-0 w-[75vw] max-w-[320px] z-50 md:hidden bg-white shadow-lifted flex flex-col transition-transform duration-400 ease-[cubic-bezier(0.16,1,0.3,1)] ${menuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex items-center justify-between px-6 py-5 border-b border-sand/60">
          <span className="font-display text-xl text-deep-brown italic font-normal">Torts & Twirls</span>
        ...
        <div className="flex-1 overflow-y-auto py-4">
          {navLinks.map((link, i) => (
            <Link
              key={link.path}
              to={link.path}
              className="block px-6 py-3.5 text-[0.9rem] font-sans text-warm-gray hover:text-deep-brown hover:bg-cream transition-colors border-b border-sand/30"
        ...
        {!isAuthenticated && (
          <div className="p-5 border-t border-sand/60 flex gap-3">
        ...
        <div className={`relative bg-white rounded-2xl shadow-lifted w-full max-w-2xl p-6 transition-all duration-400 ease-[cubic-bezier(0.16,1,0.3,1)] ${searchOpen ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-95 -translate-y-4'}`}>
          <form onSubmit={handleSearch} className="flex items-center gap-4 mb-5">
            ...
            <input
              ref={searchRef}
              type="text"
              placeholder="Search bedsheets, fabric, color..."
          ...
          <div className="border-t border-sand/50 pt-4">
            ...
            <div className="flex flex-wrap gap-2">
              {['Cotton Bedsheets', 'King Size', 'Silk', 'Printed Floral', 'Linen', 'White', 'Luxury'].map(tag => (
                <button
                  key={tag}
                  onClick={() => { setSearchQuery(tag); navigate(`/products?search=${encodeURIComponent(tag)}`); setSearchOpen(false); }}
                  className="text-xs px-3.5 py-2 bg-cream text-warm-gray rounded-full hover:bg-sand hover:text-deep-brown transition-colors font-sans border border-sand/50"
                >
                  {tag}
                </button>
              ))}
